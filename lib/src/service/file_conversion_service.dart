...
class FileConversionService {
  ...
  Project _fromTwData() {
    ...
    switch(gttPattern.type) {
      case GttPattern.PATTERN_TYPE_THREADED:
        ...
        break;
      case GttPattern.PATTERN_TYPE_DOUBLE_FACE://|BrokenTwill|Brocade|LetteredBand}>'
    ...
    jttProject.deck = gttCards.asMap().entries.map(
            (e) => _convertGttCardToTablet(e.key,e.value)
    ).toList();
    ...
    gttPicks.forEach((actionParent) {
      ...
      actions.forEach((action) {
        if (action.target==Target.CARD) {
          _processTurn(action, jttProject.deck[action.targetId-1], twists.contains(action.targetId));
        } else if (action.target==Target.PACK) {
  ...
  TwData _fromJttProject() {
    final cardData = jttProject.deck.map(
            (tablet) => _convertJttTabletToGttCard(tablet)).toList();
  ...
  Tablet _convertGttCardToTablet(int index, Card gttCard) {
    return Tablet(_convertGttHolesToThreads(gttCard.holes),
  ...
  String convertJttPatternTypeToGtt(String jttType) {
    switch(jttType) {
      case 'threadedIn': return GttPattern.PATTERN_TYPE_THREADED;
      case 'doubleWeave': return GttPattern.PATTERN_TYPE_DOUBLE_FACE;
      case 'brokenTwill':  return GttPattern.PATTERN_TYPE_BROKEN_TWILL;
      default: return null;
    }
  }
  ...
  Palette convertJttPaletteTypeToGtt(Map<String, List<String>> palettes) {
    ...
    final colours = palettes.values.first.asMap().entries.map(
            (entry) => Colour (
                index:(entry.key+1),
  ...
  Picks convertJttTabletsTypeToGttPicks(final List<Tablet> deck) {
    ...
    for (var i=0; i<pickSize; i++) {
      ...
      deck.asMap().forEach((tabletIndex, tablet) {
        final prevTwist = i==0?tablet.startingTwist:tablet.picks[i-1].twist;
        final pick = tablet.picks[i];
        final actionType = pick.twist != prevTwist?Type.TWIST:Type.TURN;
        final turned = pick.turned == Tablet.TURNING_DIRECTION_FORWARDS ? Dir.F
            : pick.turned == Tablet.TURNING_DIRECTION_BACKWARDS ? Dir.B : Dir.I;
        actions.add(Action(type: actionType,
            dist: 1,
            dir: turned,
  ...
  Packs convertJttPacksToGtt(Map<String, List<int>> jttPacks) {
    final gttPacksList = jttPacks.entries.map( (entry) => Pack(
      name: entry.key,
